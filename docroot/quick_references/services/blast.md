...
## Overview
The BLAST service integrates the BLAST (Basic Local Aligment Search Tool) algorithms to perform searches against against public or private genomes in dxkb or other reference databases using a DNA or protein sequence and find matching genomes, genes, RNAs, or proteins.  

## See also
* [BLAST Service](https://www.dxkb.org/app/Homology)
* [BLAST Service Tutorial](/tutorial/blast/blast)

## Using the BLAST Service
The **BLAST** submenu option under the **Services** main menu (Genomics category) opens the BLAST input form (*shown below*). *Note: You must be logged into dxkb to use this service.*
...
There are four BLAST programs provided by dxkb, and each has a specific query sequence and database. Clicking on the button in front of the program name will select it and will also select the appropriate databases.
...
There are three types of Query sources that are provided by dxkb:
...
dxkb has different databases to choose from for the source to search wihin: 
...
* **Genome Sequences (NT)** - Genomic sequences from bacterial and viral genomes in dxkb, i.e. chromosomes, contigs, plasmids, segments, and partial genomic sequences 
* **Genes (NT)** - Gene sequences from bacterial and viral genomes in dxkb. 
* **Proteins (AA)** - Protein sequences from bacterial and viral genomes in dxkb.
...
* **blast_headers.txt** - column headers in BLAST results table, which include
  ...
  * evalue: expect value
  * bitscore: bit score
* **blast_out.txt** - Entire file generated by BLAST job. Used by dxkb to create the blast_out.json file.
* **blast_out.json** -  JavaScript Object Notation (JSON) formatted file, which is a standard data interchange format. It is primarily used for transmitting data between the dxkb web application and backend servers.
* **blast_out.metadata.json** - JSON-formatted file containing the metadata associated with the BLAST job.
* **blast_out.raw.json** - JSON-formatted file containing list of BLAST identifiers (BLAST uses a different set of identifiers than dxkb). 
...
* **Pathway:** Loads the Pathway Summary Table containing a list of all the pathways in dxkb in which the selected features are found.
