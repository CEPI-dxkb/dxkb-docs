...
- [Protein Structure Prediction Service](https://dxkb.org/app/PredictStructure)
...
The **Protein Structure Prediction** submenu option under the **Services** main menu (Protein Tools category) opens the prediction input form. *Note: You must be logged into dxkb to use this service.*
...
| `alphafold` | AlphaFold 2 | protein only | Built from dxkb's local databases |
...
|---|---|---|
| **None** | No MSA is supplied. | Default. Works with Auto (which will pick ESMFold for single-protein, no-MSA inputs), ESMFold, and AlphaFold 2 (which generates its own MSA from dxkb's local databases). |
...
| **Use MSA Server or Service** | dxkb computes the MSA with ColabFold (MMseqs2 against UniRef + ColabFoldDB) and feeds it to the selected engine. | When you don't have a pre-computed MSA on hand and the selected tool needs one (Boltz, OpenFold, Chai). Adds 30 s – 3 min to the job. |
...
ESMFold ignores any MSA. AlphaFold 2 ignores uploaded MSAs and always builds its own from dxkb's local databases.
