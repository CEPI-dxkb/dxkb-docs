...
## See also
* [Proteome Comparison Service](https://dxkb.org/app/SeqComparison)
* [Proteome Comparison Tutorial](/tutorial/proteome_comparison/proteome_comparison)

## Using the Proteome Comparison Service
The **Proteome Comparison** submenu option under the **Services** main menu (Protein Tools category) opens the Proteome Comparison input form (*shown below*). *Note: You must be logged into dxkb to use this service.*
...
Clicking on the View icon at the upper right portion of the job result page will display an interactive circular viewer of of the comparison results, with color-coding for protein percent identity relative to the best hit on the reference genome. Mousing over a feature (gene) will display its dxkb locus tag, and clicking on it will display the feature page for that gene.  
