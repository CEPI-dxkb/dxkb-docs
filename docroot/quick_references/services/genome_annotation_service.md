...
## Output Results
Clicking on the Jobs indicator at the bottom of the dxkb page open the Jobs Status page that displays all current and previous service jobs and their status. 
...
* **features.txt** - is a tab-delimited text file listing all the features of the genome. For each feature, it contains the dxkb ID, the location string, the feature type, the functional assignment, any alternated IDs found, and (for protein-coding genes) the protein MD5 checksum.
...
* **GenomeReport.html** - contains summary report of the genome quality, described in [Analyzing Genome Quality](/tutorial/genome_quality_report/genome_quality_report). Also contains a link to the genome in the dxkb Genome Browser.
...
* **Edit Type** Allows changing of the type of the file in terms of how dxkb interprets the content and uses it in other services or parts of the website.  Allowable types include unspecified, contigs, nwk, reads, differential expression input data, and differential expression input metadata.
